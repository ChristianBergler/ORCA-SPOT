...
--------------------------------------------
1. call/noise XXX = call-XXX or noise-XXX for target and any kind of noise signals. XXX is a placeholder for a more specific call/noise label, e.g. call-N9, noise-boat
...
class Dataset(AudioDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.possible_nocall_labels = re.compile(
            "|".join(
                [
                    "noise"
                ]
        ...
        self._logger.debug("Number of noise: {}".format(len(self.file_names) - _n_calls))
    ...
    """
    Get first element of the filename which corresponds to the label file
    """
    @staticmethod
    def _label_from_filename(file_name: str):
        label = os.path.basename(file_name).rsplit(sep="_", maxsplit=5)[0]
    ...
    """
    According to the given filename it returns whether the file is a target signal (call) or not
